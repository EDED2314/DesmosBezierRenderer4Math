...
DOWNLOAD_IMAGES = (
    False  # Download each rendered frame automatically (works best in firefox)
)
USE_L2_GRADIENT = (
    False  # Creates less edges but is still accurate (leads to faster renders)
)
...
def get_trace(data):
    ...
    path = bmp.trace(2, potrace.TURNPOLICY_MINORITY, 1.0, 1, 0.5)
    return path
...
def get_expressions(frame):
    ...
    for expr in get_latex(FRAME_DIR + "/frame%s.%s" % (use_f, FILE_EXT)):
        exprid += 1
        exprs.append(
            {
                "id": "expr-" + str(exprid),
...
def client():
    return render_template(
        "index.html",
        api_key="dcb31709b452b1cf9dc26972add0fda6",  # Development-only API_key. See https://www.desmos.com/api/v1.8/docs/index.html#document-api-keys
        height=height.value,
        width=width.value,
...
if __name__ == "__main__":
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hf:e:c:bdlg", ["static", "block=", "maxpblock=", "yes"]
        )
...
"""
        ...
        print(
            "Processing %d frames... Please wait for processing to finish before running on frontend\n"
